...
# Example
```jldoctest; setup = (import SphericalHarmonicTransforms: trillen)
julia> trillen(3)
6
...
# Example
```jldoctest; setup = (import SphericalHarmonicTransforms: trildim)
julia> trildim(6)
(3, 3)
...
# Example
```jldoctest; setup = (import SphericalHarmonicTransforms: packtril!)
...
# Example
```jldoctest; setup = (import SphericalHarmonicTransforms: unpacktril!)
...
`op` which returns `y = op(x) == A * x`.

The search is started with `x .= 0`.

For a description of the keyword arguments and other implementation details, see
[`cg!`](@ref).
"""
function cg end

"""
    (; info...) = cg!(x, A, b::AbstractVector{T};
...
method.
Optionally, the system ``A`` may be represented in "matrix-free" form as a linear operator
`op` which modifies `y` as `op!(y, x)`.

The vector `x` is not (re)initialized prior to iteration and therefore can be used to
provide an initial starting guess if an approximate solution is known.
...
The returned `info` is a `NamedTuple` with the following fields:
...
# Example
```jldoctest; setup = (import SphericalHarmonicTransforms: cg, cg!)
julia> A = [ 1.0 -0.2
            -0.2  2.0];
